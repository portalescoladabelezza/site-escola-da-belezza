  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end space-y-2">
        ...
        {/* Compact Tester Panel */}
        {showConfigPanel && (
          <motion.div 
            initial={{ opacity: 0, y: 15 }} 
            animate={{ opacity: 1, y: 0 }}
            className="bg-white border border-[#E5D1B8] shadow-xl p-4 rounded-xl text-left w-64 text-xs space-y-3 font-sans"
          >
            <div className="flex justify-between items-center border-b border-stone-100 pb-2">
              <span className="font-bold text-[#1A1A1A] flex items-center gap-1">
                <RefreshCw size={13} className="text-[#C49B66]" /> Modulador de Leads (VIP)
              </span>
              <button 
            ...
            <div className="space-y-2 text-stone-600">
              <div className="flex justify-between">
                ...
                <span className={`font-bold ${wasClosed ? 'text-amber-600' : 'text-emerald-600'}`}>
                  {wasClosed ? 'Inativo (Fechado/Salvo)' : 'Monitorando Gatilhos'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span>Rolo da Página:</span>
                <span className="font-mono bg-[#F9F6F2] px-1.5 py-0.5 rounded font-bold">
                  {currentScrollPercent}% / {scrollThreshold}%
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span>Tempo p/ Disparo:</span>
                <span className="font-mono bg-[#F9F6F2] px-1.5 py-0.5 rounded font-bold">
                  {secondsRemaining}s / {timeDelay / 1000}s
                </span>
              </div>
            </div>
            ...
            <div className="pt-2 flex gap-2">
              <button
                ...
                className="flex-1 bg-stone-100 hover:bg-[#F2E8DF] text-[#1A1A1A] font-bold py-1.5 rounded transition uppercase tracking-wider text-[9px] cursor-pointer"
              >
                Limpar Cache
              </button>
              <button
                onClick={handleForceOpen}
                className="flex-1 bg-[#C49B66] hover:bg-[#B38A55] text-white font-bold py-1.5 rounded transition uppercase tracking-wider text-[9px] cursor-pointer"
              >
                Forçar Abertura
              </button>
            </div>
        ...
        {/* Small debug trigger & official float option */}
        <div className="flex gap-2">
          {/* Debug Panel Toggle */}
          <button
            onClick={() => setShowConfigPanel(!showConfigPanel)}
            className="bg-[#2D2D2D] hover:bg-stone-800 text-[#E5D1B8] text-[9px] font-mono tracking-widest uppercase py-2 px-3.5 rounded-full shadow-lg transition flex items-center gap-1.5 border border-[#C49B66]/30 cursor-pointer"
            title="Painel de Testes do Programador"
          >

...
import { Sparkles, ArrowRight, BookOpen, Clock, Award, Star, Zap, User } from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ articles, onNavigate }) => {
  ...
  const categories = [
    { name: 'Cabelos', count: '10+ Artigos', icon: '💇‍♀️', color: 'from-amber-50 to-orange-50/50' },
    { name: 'Unhas', count: '15+ Artigos', icon: '💅', color: 'from-pink-50 to-rose-50/50' },
    { name: 'Maquiagem', count: '12+ Artigos', icon: '💄', color: 'from-rose-50 to-purple-50/50 text-stone-800' },
    { name: 'Sobrancelhas e Cílios', count: '8+ Artigos', icon: '👁️', color: 'from-amber-50 to-yellow-50/50' },
    { name: 'Estética Facial', count: '10+ Artigos', icon: '✨', color: 'from-blue-50 to-cyan-50/50' },
    { name: 'Carreira', count: '14+ Artigos', icon: '💼', color: 'from-stone-50 to-stone-100/50' },
    { name: 'Empreendedorismo', count: '20+ Artigos', icon: '📈', color: 'from-[#FFF5F0] to-[#FAF2EE]' },
    { name: 'Tendências', count: '6+ Artigos', icon: '🔥', color: 'from-[#FDF2F8] to-[#FFF5F5]/40' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="col-span-1 lg:col-span-7 space-y-6 text-center lg:text-left">
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-stone-200/80 dark:border-[#332C28] max-w-sm sm:max-w-md mx-auto lg:mx-0">
                <div>
                  <span className="block text-xl sm:text-2xl font-bold text-[#1A1A1A] dark:text-[#EDE6E2]">12</span>
                  <span className="text-[10px] sm:text-xs font-sans text-stone-500 dark:text-[#A89D96]">Guias Completos</span>
                </div>
                <div>
                  <span className="block text-xl sm:text-2xl font-bold text-[#1A1A1A] dark:text-[#EDE6E2]">30k+</span>
                  <span className="text-[10px] sm:text-xs font-sans text-stone-500 dark:text-[#A89D96]">Leitoras Mensais</span>
                </div>
                <div>
                  <span className="block text-xl sm:text-2xl font-bold text-[#C49B66]">100%</span>
                  <span className="text-[10px] sm:text-xs font-sans text-stone-500 dark:text-[#A89D96]">Prático & Grátis</span>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          {categories.map((cat, index) => (
            <div
              ...
              className={`p-5 rounded-2xl bg-gradient-to-tr ${cat.color} dark:from-[#221D1A] dark:to-[#1C1816] border border-stone-150/60 dark:border-[#332C28]/80 shadow-xs hover:shadow-md transition duration-200 cursor-pointer hover:scale-[1.01]`}
            >
              <span className="text-3xl block" role="img" aria-label={cat.name}>
                {cat.icon}
              </span>
              <h3 className="text-xs sm:text-sm font-sans font-bold text-stone-800 dark:text-[#EDE6E2] mt-3 uppercase tracking-wide">
                {cat.name}
              </h3>
              <div className="flex items-center space-x-1 text-[10px] sm:text-xs text-[#C5A880] mt-1 font-semibold">
                <span>{cat.count}</span>
                <ArrowRight size={11} className="text-[#C5A880] shrink-0" />
              </div>
            </div>
          ))}
        </div>
